...
int render_text(uint16_t * buffer)
{
    ...
    mf_render_aligned(font, options.anchor, state.y,
                      options.alignment, options.text, strlen(options.text),
                      character_callback, &state);

    /*
    mf_render_justified(font, options.anchor, state.y,
                        state.width - options.margin * 2,
                        options.text, strlen(options.text), character_callback, &state);
